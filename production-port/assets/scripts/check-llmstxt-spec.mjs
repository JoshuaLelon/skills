...
try {
	...
	if (hash === SPEC_HASH) {
		...
		console.log(`llmstxt-spec: UPSTREAM CHANGED (current ${hash.slice(0, 12)}…, pinned ${SPEC_HASH.slice(0, 12)}…).`);
		console.log("  1. Read https://llmstxt.org/ and diff against what docs-index.mjs generates.");
		console.log("  2. Update scripts/docs-index.mjs AND the production-port skill's assets/scripts/ copy.");
	...
	console.log(`llmstxt-spec: could not reach ${SPEC_URL} (${e.message ?? e}) — skipping, not failing.`);
}
...
try {
	...
	if (masters.length) {
		...
		if (PLAYBOOKS_HASH === "PENDING") {
			...
			console.log("  (AGENTS-template §4, references/react-patterns.md, references/testing.md), then bump PLAYBOOKS_HASH.");
		}
	}
} catch {
