...
if (tsconfigs.length && !tsconfigs.some((f) => /"erasableSyntaxOnly"\s*:\s*true/.test(read(f) ?? ""))) {
	problems.push("no tsconfig sets erasableSyntaxOnly:true — enums/namespaces from training data will emit runtime code");
}
...
for (const f of readdirSync(ROOT).filter((f) => /^(vitest|vite)\.config\./.test(f))) {
	if (/thresholds/.test(read(f) ?? "")) problems.push(`${f}: coverage thresholds — a gated metric gets gamed; remove`);
}
...
for (const [name, cmd] of Object.entries(scripts)) {
	if (/wrangler\s+deploy\s+--env/.test(cmd))
		problems.push(`package.json scripts.${name}: 'wrangler deploy --env' silently deploys to PROD — select env at build time (CLOUDFLARE_ENV=… npm run build)`);
	if (/wrangler\s+deploy/.test(cmd) && !/build/.test(cmd) && !/npm run (check|build)/.test(cmd))
		problems.push(`package.json scripts.${name}: 'wrangler deploy' without a build ships the stale ./build`);
}
if (hasWrangler && !scripts.deploy) {
	problems.push("wrangler installed but no scripts.deploy — deploys must be the scripted path (check && build && wrangler deploy), never bare wrangler");
}
...
if (example) {
	...
	const scan = (dir) => {
		...
		for (const n of names) {
			...
			for (const m of (read(p) ?? "").matchAll(/(?:process\.env|import\.meta\.env)\.([A-Z][A-Z0-9_]+)/g))
	...
	for (const v of [...used].sort())
		if (!declared.has(v))
			problems.push(`env var ${v} is read by the app but missing from .dev.vars.example — the manifest must list every var, set or not`);
}
...
if (wranglerRaw) {
	...
	if (!/"observability"/.test(wranglerRaw))
		problems.push("wrangler: no observability block — Workers Logs is the entire log/error backend (ADR-0009); enable it");
	if (!/"upload_source_maps"\s*:\s*true/.test(wranglerRaw))
		problems.push("wrangler: upload_source_maps not enabled — production stack traces stay minified (ADR-0009)");
	try {
		const jsonc = wranglerRaw.replace(/\/\*[\s\S]*?\*\//g, "").replace(/^\s*\/\/.*$/gm, "").replace(/,(\s*[}\]])/g, "$1");
		const w = JSON.parse(jsonc);
		for (const [envName, env] of Object.entries(w.env ?? {})) {
			if (env.compatibility_date)
				problems.push(`wrangler env.${envName}: compatibility_date must be top-level only — environments inherit it`);
			const top = Object.keys(w.vars ?? {});
			const envVars = new Set(Object.keys(env.vars ?? {}));
			for (const k of top)
				if (!envVars.has(k))
					problems.push(`wrangler env.${envName}.vars is missing '${k}' — vars does NOT inherit; every var must be repeated per env or it silently vanishes there`);
		}
	} catch {
