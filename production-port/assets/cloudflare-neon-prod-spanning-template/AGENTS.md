...
33 rules that FAIL a build: 11 ast-grep, 6 depcruise, 16 gate. dependency-cruiser's
...
| `no-unrebased-fixture-timestamp` | Fixture timestamps are anchored on NOW; a row reaching the database must be rebased onto the seeder's `now` — `new Date(now.getTime() + …)`, which is what rebase() in src/db/seed.ts does. | ast-grep | ADR-0005 |
