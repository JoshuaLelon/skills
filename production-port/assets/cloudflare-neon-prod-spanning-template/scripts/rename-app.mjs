...
w = w.replaceAll('"spanning-template-demo"', `"${name}-demo"`).replaceAll('"spanning-template"', `"${name}"`);
w = w.replace(/"id": "[0-9a-f]{32}"/, '"id": "00000000000000000000000000000000" /* [FILL: npx wrangler hyperdrive create] */');
...
writeFileSync("README.md", r.replace(`# ${TEMPLATE}`, `# ${name}\n\n[FILL: what this app is]\n\n<details><summary>Template provenance</summary>`) + "\n</details>\n");

console.log(`rename-app: ${TEMPLATE} → ${name}. Now: node scripts/docs-index.mjs && provision your own Hyperdrive (environments.md).`);
