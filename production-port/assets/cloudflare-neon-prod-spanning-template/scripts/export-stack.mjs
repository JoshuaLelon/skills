...
if (!FROM) {
	console.error(
		'export-stack: pass --from <proven-repo> (or --stamp). e.g. --from ~/workspace/pantogen',
	)
	process.exit(1)
}

const src = JSON.parse(
	readFileSync(join(FROM.replace(/^~/, process.env.HOME), 'package.json'), 'utf8'),
)
const fmt = (o) =>
	Object.entries(o ?? {})
		.map(([n, v]) => `${n}@${v.replace(/^[\^~]/, '')}`)
...
if (APPLY) {
	...
	console.log(
		'export-stack: applied. Run `node scripts/export-stack.mjs --stamp` to update AGENTS.md §1.',
	)
}
