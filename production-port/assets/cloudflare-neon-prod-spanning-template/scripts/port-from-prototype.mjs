...
if (STATUS) {
	...
	if (exemplar.length) {
		console.log(
			`port:status — exemplar still present (delete once its pattern has a real follower):`,
		)
...
if (fromIdx === -1 || !args[fromIdx + 1]) {
	console.error(
		'usage: node scripts/port-from-prototype.mjs --from <prototype-repo>   (or --status)',
	)
	process.exit(1)
}
...
try {
	...
	console.error(
		`port: ${SRC} has no 'prototype' tag — run the prototyping skill's Phase 7 first (tag, then strip).`,
	)
	process.exit(1)
}
if (existsSync(join(SRC, 'src/walkthrough.tsx'))) {
	console.error(
		`port: ${SRC} still has the walkthrough harness — run strip-harness.mjs there first.`,
	)
	process.exit(1)
}
